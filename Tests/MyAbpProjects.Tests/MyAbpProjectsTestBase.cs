...
using MyAbpProjects.Migrations.SeedData;
...
namespace MyAbpProjects.Tests
{
    public abstract class MyAbpProjectsTestBase : AbpIntegratedTestBase<MyAbpProjectsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyAbpProjectsTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyAbpProjectsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyAbpProjectsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyAbpProjectsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyAbpProjectsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyAbpProjectsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpProjectsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyAbpProjectsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpProjectsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyAbpProjectsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyAbpProjectsDbContext, Task<T>> func)
        {
            T result;
